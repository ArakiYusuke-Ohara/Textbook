...
// マップ衝突判定用のプレイヤーサイズ補正
#define PLAYER_MAP_COLLISION_OFFSET (0.1f)
// 矩形判定の位置補正
...
void DrawPlayer()
{
	...
	DrawAnimation(animData, g_PlayerData.posX, g_PlayerData.posY, g_PlayerData.isTurn);

	// 当たり判定の描画
	//float x, y, w, h;
	//CalcBoxCollision(g_PlayerData, x, y, w, h);
	//DrawBox((int)x, (int)y, (int)(x + w), (int)(y + h), GetColor(255, 255, 255), false);
}
...
void PlayerHitNormalBlock(MapChipData mapChipData)
{
