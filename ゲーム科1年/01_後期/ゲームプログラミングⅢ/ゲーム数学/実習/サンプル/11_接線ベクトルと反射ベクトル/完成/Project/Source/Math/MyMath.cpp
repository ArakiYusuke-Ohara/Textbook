...
VECTOR VecTangent(VECTOR vecIncident, VECTOR vecNormal)
{
	// ① 入射ベクトルを反転する
	VECTOR reversIncident = VecScale(vecIncident, -1.0f);

	// ② ①と法線ベクトルの内積を計算する
	float dot = VecDot(reversIncident, vecNormal);

	// ③ 向きは法線ベクトル、長さは②のベクトルを用意する
	VECTOR afterPos = VecScale(vecNormal, dot);

	// ④ 接線ベクトルは 入射ベクトル + ③のベクトル
	return VecAdd(vecIncident, afterPos);
}

VECTOR VecReflact(VECTOR vecIncident, VECTOR vecNormal)
{
	// ① 入射ベクトルを反転する
	VECTOR reversIncident = VecScale(vecIncident, -1.0f);
	...
	// ③ 向きは法線ベクトル、長さは②を2倍にしたベクトルを用意する
	VECTOR afterPos = VecScale(vecNormal, dot * 2.0f);

	// ④ 反射ベクトルは 入射ベクトル + ③のベクトル
	return VecAdd(vecIncident, afterPos);
}
