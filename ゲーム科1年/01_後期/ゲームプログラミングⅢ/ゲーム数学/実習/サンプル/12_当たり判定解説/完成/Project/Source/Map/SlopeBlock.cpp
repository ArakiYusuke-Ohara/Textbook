...
{
	// 坂道の始点（左側）
	float startX = block->body.pos.x;
	float startY = block->body.pos.y + block->body.width;
	// 坂道の終点（右側）
	float endX = block->body.pos.x + block->body.width;
	float endY = block->body.pos.y;
	// プレイヤーの足先のX座標
	float playerFootX = body->pos.x + body->width;
	// プレイヤーの足先座標は坂の端を超えてはいけない
	if (playerFootX < block->body.pos.x) playerFootX = block->body.pos.x;
	if (playerFootX > (block->body.pos.x + block->body.width)) playerFootX = (block->body.pos.x + block->body.width);
