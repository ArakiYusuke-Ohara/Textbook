...
void Player::Load()
{
	...
	// アニメーションクラス生成
	m_Animation = MakeUnique<Animation>(m_Handle);
}
...
void Player::Start()
{
	...
	// 待機アニメーションを再生
	m_Animation->Play(PLAYER_ANIMATION_WAIT, ANIMATION_SPEED, true);
}
...
void Player::Step()
{
	...
	if (Input::IsInputKey(KEY_UP))
	{
		...
		// 歩行アニメーション再生
		m_Animation->Play(PLAYER_ANIMATION_WALK, ANIMATION_SPEED, true);
	}
	else
	{
