...
void CheckCollision()
{
	...
	{
		// 中心座標を計算
		float enemyCenterX = waterEnemy->posX + WATER_ENEMY_WIDTH / 2;
		float enemyCenterY = waterEnemy->posY + WATER_ENEMY_HEIGHT / 2;

		if (CheckCircleCircle(playerCenterX, playerCenterY, PLAYER_RADIUS, enemyCenterX, enemyCenterY, WATER_ENEMY_RADIUS))
		{
			// 当たった時のプレイヤー側の処理
			PlayerHitWaterEnemy();
			// 当たった時の水色エネミー側の処理
			WaterEnemyHitPlayer(i);
		}
	}
}
