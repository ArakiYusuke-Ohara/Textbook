...
void CheckPlayerFlies()
{
	...
	if (player->active && !player->invisible)
	{
		...
		for (int i = 0; i < ENEMY_FLIES_MAX; i++, flies++)
		{
			...
			// Fliesの中心座標
			float fliesCenterX = flies->base.posX + ENEMY_FLIES_WIDTH / 2;
			float fliesCenterY = flies->base.posY + ENEMY_FLIES_HEIGHT / 2;
...
void CheckPlayerDragonFly()
{
	...
	if (player->active && !player->invisible)
	{
		...
		for (int i = 0; i < ENEMY_MAX; i++, dragonFly++)
		{
			// 死んでいたら判定しない
			if (!dragonFly->base.active) continue;

			// DragonFlyの中心座標
			float dragonFlyCenterX = dragonFly->base.posX + ENEMY_DRAGON_FLY_WIDTH / 2;
			float dragonFlyCenterY = dragonFly->base.posY + ENEMY_DRAGON_FLY_HEIGHT / 2;
...
void CheckPlayerKapu()
{
	...
	if (player->active && !player->invisible)
	{
		...
		for (int i = 0; i < ENEMY_MAX; i++, kapu++)
		{
			// 死んでいたら判定しない
			if (!kapu->base.active) continue;

			// DragonFlyの中心座標
			float kapuCenterX = kapu->base.posX + ENEMY_KAPU_WIDTH / 2;
			float kapuCenterY = kapu->base.posY + ENEMY_KAPU_HEIGHT / 2;
