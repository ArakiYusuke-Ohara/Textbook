...
void LadingPlayer();
// 押し出し処理
void PlayerPushOutX(float targetX, float targetY, float targetW, float targetH, bool checkRight, bool checkLeft);
void PlayerPushOutY(float targetX, float targetY, float targetW, float targetH);
...
void PlayerPushOutY(float targetX, float targetY, float targetW, float targetH)
{
		...
		targetX, targetY, targetW, targetH))
	{
		...
		{
			// 下に押し出す
			g_PlayerData.posY += (targetY + targetW) - y;
			// Y移動量を0にする
			g_PlayerData.moveY = 0.0f;
		}
	}
