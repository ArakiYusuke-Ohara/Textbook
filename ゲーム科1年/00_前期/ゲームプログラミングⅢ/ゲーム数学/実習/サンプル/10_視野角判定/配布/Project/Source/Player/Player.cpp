...
void StartPlayer()
{
	...
	// 矩形判定設定
	g_PlayerData.boxCollision.pos.x = PLAYER_BOX_COLLISION_OFFSET_X;
	g_PlayerData.boxCollision.pos.y = PLAYER_BOX_COLLISION_OFFSET_Y;
...
void UpdatePlayer()
{
	...
	// 落ちている
	else if (g_PlayerData.move.y > PLAYER_FALL_Y_MOVE)
	{
		SetState(PLAYER_AIR);
...
void DrawPlayer()
{
	...
	VECTOR drawPos = VGet(g_PlayerData.pos.x - camera.pos.x, g_PlayerData.pos.y - camera.pos.y, 0.0f);
	DrawAnimation(animData, drawPos, g_PlayerData.isTurn);

	// 画像範囲を描画
	DrawBox((int)(g_PlayerData.pos.x - camera.pos.x), (int)(g_PlayerData.pos.y - camera.pos.y), 
		(int)(g_PlayerData.pos.x + PLAYER_WIDTH - camera.pos.x), (int)(g_PlayerData.pos.y + PLAYER_HEIGHT - camera.pos.y), GetColor(255, 255, 255), false);

	// 当たり判定を描画
	//float x, y, w, h;
	//CalcBoxCollision(g_PlayerData, x, y, w, h);
	//DrawBox((int)(x - camera.pos.x), (int)(y - camera.pos.y), (int)(x + w - camera.pos.x), (int)(y + h - camera.pos.y), GetColor(255, 255, 255), false);

	DrawFormatString(0, 0, GetColor(255, 255, 255), "プレイヤーの座標＝[%f, %f]", g_PlayerData.pos.x, g_PlayerData.pos.y);
}
...
bool PlayerPushOutX(float targetX, float targetY, float targetW, float targetH, bool checkRight, bool checkLeft)
{
	...
	if (CheckSquareSquare(VGet(x + POS_OFFSET, y + POS_OFFSET, 0.0f), w - SIZE_OFFSET, h - SIZE_OFFSET,
		VGet(targetX, targetY, 0.0f), targetW, targetH))
	{
		isHit = true;

		// 左からあたったか
		if (checkLeft && player.move.x > 0.0f)
		{
			// 左に押し出す
			g_PlayerData.pos.x -= (x + w) - targetX;
		}
		// 右からあたったか
		else if (checkRight && player.move.x < 0.0f)
		{
			// 右に押し出す
			g_PlayerData.pos.x += (targetX + targetW) - x;
		}
	}
...
bool PlayerPushOutY(float targetX, float targetY, float targetW, float targetH, float attract)
{
	...
	player.pos.x = g_PrevPlayerData.pos.x;
	player.pos.y = g_PlayerData.pos.y;
	...
	// まだ当たっているなら縦に当たっている
	if (CheckSquareSquare(VGet(x + POS_OFFSET, y + POS_OFFSET, 0.0f), w - SIZE_OFFSET, h - SIZE_OFFSET,
		VGet(targetX, targetY - attract, 0.0f), targetW, targetH + attract))
	{
		isHit = true;
		...
		if ((y + h) >= targetY - attract && (y + h) <= (targetY + targetH + attract))
		{
			...
			LandingPlayer();
			// 上に押し出す
			g_PlayerData.pos.y -= (y + h) - targetY;
		...
		{
			// 下に押し出す
			g_PlayerData.pos.y += (targetY + targetH) - y;
...
void StartPlayerAnimation(PlayerAnimationType anim)
{
	...
	// アニメーション再生
	StartAnimation(animData, g_PlayerData.pos,animParam.interval, animParam.frameNum, animParam.width, animParam.height, true);
}
...
void UpdatePlayerAnimation()
{
	...
	{
		// 横に移動していれば走っている
		if (g_PlayerData.move.x < 0.0f || g_PlayerData.move.x > 0.0f)
		{
			// 走るアニメーション
...
{
	x = player.isTurn ?
		player.pos.x + PLAYER_WIDTH - player.boxCollision.pos.x - player.boxCollision.width :
		player.pos.x + player.boxCollision.pos.x;
	y = player.pos.y + player.boxCollision.pos.y;
...
{
	PlayerPushOutX(autoDoor.pos.x, autoDoor.pos.y, AUTO_DOOR_WIDTH, AUTO_DOOR_HEIGHT, true, true);
}

void PlayerHitAutoDoorY(AutoDoorData autoDoor)
{
	PlayerPushOutY(autoDoor.pos.x, autoDoor.pos.y, AUTO_DOOR_WIDTH, AUTO_DOOR_HEIGHT);
}

void PlayerHitMoveBlockX(MoveBlockData moveBlock)
{
	PlayerPushOutX(moveBlock.pos.x, moveBlock.pos.y, ARCH_BLOCK_WIDTH, ARCH_BLOCK_HEIGHT, true, true);
}

void PlayerHitMoveBlockY(MoveBlockData moveBlock)
{
	bool isHit = PlayerPushOutY(moveBlock.pos.x, moveBlock.pos.y, ARCH_BLOCK_WIDTH, ARCH_BLOCK_HEIGHT, 8.0f);

	// ヒットしていて
	if (isHit)
	{
		// 上に乗っていたら横移動を追従させる
		if (g_PlayerData.move.y >= 0.0f && !IsState(PLAYER_AIR))
		{
			g_PlayerData.pos.x += moveBlock.pos.x - moveBlock.oldPos.x;
		}
