...
void StartAnimationEffect(AnimationEffectType type, VECTOR pos, int interval, bool isLoop)
{
	// マスターパラム取得
	...
	for (int i = 0; i < ANIMATION_EFFECT_MAX; i++, effect++)
	{
		...
		if (!effect->active)
		{
			StartAnimation(effect, pos, interval, param.frameNum, param.frameWidth, param.frameHeight, isLoop);
			break;
		}
	}
}
