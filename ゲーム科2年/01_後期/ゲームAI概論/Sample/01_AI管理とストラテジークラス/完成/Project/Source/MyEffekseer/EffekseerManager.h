...
public:
	...
	// エフェクト再生
	EffekseerEffectBase* PlayEffect2D(int type, VECTOR pos, VECTOR scale = {1.0f, 1.0f, 1.0f});
	EffekseerEffectBase* PlayEffect3D(int type, VECTOR pos, VECTOR scale = {1.0f, 1.0f, 1.0f});
...
private:
	...
	int* m_EffectHandls;		// リソースハンドル配列
	std::vector<EffekseerEffectBase*> m_Effects;	// エフェクト配列
};
