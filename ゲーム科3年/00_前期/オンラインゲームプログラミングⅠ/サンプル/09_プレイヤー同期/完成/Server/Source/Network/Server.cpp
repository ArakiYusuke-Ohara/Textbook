...
bool Server::ReceiveData()
{
	...
	for (const NetworkPlayerData& player : m_NetworkPlayerData)
	{
		...
		if (dataLength > 0)
		{
			...
			switch (header.type)
			{
				case PacketType::POS:		SyncPos(player.client.handle); break;	// 座標を同期
				case PacketType::ROT:		SyncRot(player.client.handle); break;	// 回転を同期
				case PacketType::SCALE:		SyncScale(player.client.handle); break;	// 拡縮を同期
